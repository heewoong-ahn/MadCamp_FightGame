...
public class enemyPlayer : MonoBehaviour
{
    ...
    {
        return !IsGuardAnimationPlaying() && !IsBasicPunchAnimationPlaying() && !IsRightPunchAnimationPlaying() && !IsUpperCutAnimationPlaying() && !IsLeftKickAnimationPlaying() && !IsLeftKickAnimationPlaying() && !IsUnderKickAnimationPlaying() && !IsRightKickAnimationPlaying() && !IsBasicPunchedAnimationPlaying() && !IsKickedAnimationPlaying() && !IsUnderKickedAnimationPlaying();
    }

    private void MoveForward()
    {
        if (Input.GetKeyDown(KeyCode.LeftArrow) && timer < 0)
        {
        ...
        // 키가 눌린 상태이면 걷기 동작
        if (Input.GetKey(KeyCode.LeftArrow) && !IsBasicPunchAnimationPlaying() && !IsLeftKickAnimationPlaying() && !IsRightKickAnimationPlaying() && !IsUnderKickAnimationPlaying() && !IsBasicPunchedAnimationPlaying() && !IsKickedAnimationPlaying() && !IsUnderKickedAnimationPlaying() && !IsGuardAnimationPlaying() && !IsRightPunchAnimationPlaying() && !IsUpperCutAnimationPlaying() && timer < 0)
        {
    ...
    private void MoveBackward()
    {
        ...
        // 키가 눌린 상태이면 걷기 동작
        if (Input.GetKey(KeyCode.RightArrow) && !IsBasicPunchAnimationPlaying() && !IsLeftKickAnimationPlaying() && !IsRightKickAnimationPlaying() && !IsUnderKickAnimationPlaying() && !IsBasicPunchedAnimationPlaying() && !IsKickedAnimationPlaying() && !IsUnderKickedAnimationPlaying() && !IsGuardAnimationPlaying() && !IsRightPunchAnimationPlaying() && !IsUpperCutAnimationPlaying() && timer < 0)
        {
    ...
    {
        // "Base Layer"�� Animator Controller�� �⺻ ���̾� �̸��Դϴ�.
        int BaseLayerIndex = anim.GetLayerIndex("Base Layer");
